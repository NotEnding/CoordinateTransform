...
def get_lng_lat_index(headers):
    try:
        ...
        print('Error: ' + str(error).split('is', 1)[
            0] + 'is missing from csv header. Or use -n or -a to specify custom column name for lng or lat.')
        sys.exit()


def convert_by_type(lng, lat, type):
    if type == 'g2b':
        return cdt.gcj02_to_bd09(lng, lat)
    elif type == 'b2g':
        return cdt.bd09_to_gcj02(lng, lat)
    elif type == 'w2g':
        return cdt.wgs84_to_gcj02(lng, lat)
    elif type == 'g2w':
        return cdt.gcj02_to_wgs84(lng, lat)
    elif type == 'b2w':
        return cdt.bd09_to_wgs84(lng, lat)
    elif type == 'w2b':
        return cdt.wgs84_to_bd09(lng, lat)
    else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Convert coordinates in csv files.',
                                     usage='%(prog)s [-h] -i INPUT -o OUTPUT -t TYPE [-n LNG_COLUMN] [-a LAT_COLUMN] [-s SKIP_INVALID_ROW]',
    ...
    group.add_argument('-t', '--type', help='Convert type, must be one of: g2b, b2g, w2g, g2w, b2w, w2b',
    ...
    group.add_argument('-s', '--skip_invalid_row', help='Whether to skip invalid row', default=False, type=bool,
                       metavar='')
